...
mod5M4 <- quap(
  alist(
    ...
    mu <- a + bA*MedianAgeMarriage.std + bM*Marriage.std + bL*LDSpct.std,
    a ~ dnorm(0, 0.2),
    bA ~ dnorm(0, 0.5),
    bM ~ dnorm(0, 0.5),
    bL ~ dnorm(0, 1),
...
```{r}
# xseq <- seq( from= , to=, length.out=100 )
# 
# pred <- link( mod5M4 , data=list( N=xseq) )
# 
# pred.mu <- apply( pred, 2, mean )
# pred.HPDI <- apply( pred, 2, HPDI, .89 )
# sim.divorce <- sim( mod5M4 )
# sim.HPDI <- apply( sim.divorce, 2, HPDI, prob=0.89 )
# 
# plot(d$Divorce ~ pred.mu, data = d)
# lines(weight.seq, mu.mean)
# shade(mu.HPDI, weight.seq)
# shade(sim.HPDI, weight.seq)
# 
# xseq <- seq( from=min(d$)-0.15 , to=max(dcc$N)+0.15 , length.out=30 )
# mu <- link( mod5M4 , data=list(N=xseq) )
...
groupsz.seq <- seq(from = 1, to = 9, length.out = 100)
mu <- link(mod5H1.2, data = data.frame(area = groupsz.seq))
mu.mean <- apply(mu, 2, FUN = mean)
mu.PI <- apply(mu, 2, FUN = PI, prob = .89)
